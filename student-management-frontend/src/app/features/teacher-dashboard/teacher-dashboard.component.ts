...
export class TeacherDashboardComponent implements OnInit {
  ...
  loadAllStudents(): void {
    ...
    this.http.get<Student[]>(`${this.api}/students/getAll.php`).subscribe({
      next: (students) => {
  ...
  loadGroupsForCourse(): void {
    ...
    this.http.get<Group[]>(`${this.api}/student_groups/getAll.php`).subscribe({
      next: (groups) => {
        // Get groups that have this course
        this.http.get<any[]>(`${this.api}/group_courses/getByCourse.php?course_id=${this.selectedCourse!.id}`).subscribe({
          next: (groupCourses) => {
            const groupIds = groupCourses.map(gc => gc.group_id);
            this.groups = groups.filter(g => groupIds.includes(g.id));
          }
        });
      }
    });
  ...
  loadStudentsForGroup(): void {
    ...
    this.loading = true;
    this.http.get<Student[]>(`${this.api}/students/getByGroup.php?group_id=${this.selectedGroup!.id}`).subscribe({
      next: (students) => {
  ...
  autoSelectFirstAvailableLab(): void {
    if (this.laboratories.length === 0) return;

    // Find first lab that is not completed
    const firstAvailable = this.laboratories.find(lab => !this.isLabCompleted(lab.lab_number));
    if (firstAvailable) {
      this.selectedLaboratory = firstAvailable;
    }
  }
  ...
  loadExistingSessions(): void {
    ...
    this.http.get<Session[]>(`${this.api}/sessions/getByCourse.php?course_id=${this.selectedCourse.id}`).subscribe({
      next: (sessions) => {
        // Filter sessions for this specific group
        this.sessions = sessions.filter(s => s.group_id === this.selectedGroup!.id);
      }
    });
  }
  ...
  createSession(): void {
    ...
    this.http.post<{ id: number }>(`${this.api}/sessions/create.php`, sessionData).subscribe({
      next: (response) => {
  ...
  markAttendance(sessionId: number): void {
    ...
    this.http.post(`${this.api}/attendance/save.php`, attendanceData).subscribe({
      next: () => {
        this.loading = false;
        this.error = '';
        alert(`Laboratory ${this.selectedLaboratory!.lab_number} attendance recorded successfully!`);
        this.resetForm();
        this.loadExistingSessions();
      },
      error: (error) => {
  ...
  getStudentGroupName(student: Student): string {
    return `Group ${student.group_name}`;
  }
}
